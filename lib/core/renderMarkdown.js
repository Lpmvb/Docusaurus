...
function codePlayground(md) {
  md.renderer.rules.fence_custom = {
    playground: function(tokens, idx, options, env, instance) {
      ...
      if (options.highlight) {
        highlighted =
          options.highlight.apply(
            options.highlight,
            [token.content].concat(fences)
          ) || escapeHtml(token.content);
      } else {
        highlighted = escapeHtml(token.content);
      }
      const frame = `<iframe class="code-playground" width="100%" srcdoc="${
        escapeHtml(tokens[idx].content)
      }" frameborder="0" scrolling="no" onload="this.style.height=this.contentDocument.body.scrollHeight +'px';"></iframe>`;
      return `<pre><code class="hljs html css">${highlighted}</code></pre>${frame}`;
    },
  };
}
